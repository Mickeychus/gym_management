...
public class HuanLuyenVienView extends JFrame implements ActionListener, ListSelectionListener {
...
//	private JTextField gpaField;
	...
	private String[] columnNames = new String[] { "Mã huấn luyện viên", "Họ tên", "Tuổi", "Địa chỉ" };
	// định nghĩa dữ liệu mặc định của bẳng student là rỗng
	private Object data = new Object[][] {};

	public HuanLuyenVienView() {
		initComponents();
	}
...
//		sortStudentGPABtn = new JButton("Sort By GPA");
		...
		// khởi tạo các label
		idLabel = new JLabel("Mã HLV");
		nameLabel = new JLabel("Học tên");
...
//		layout.putConstraint(SpringLayout.WEST, sortStudentNameBtn, 115, SpringLayout.WEST, sortStudentGPABtn);
		...
		this.add(panel);
		this.pack();
		this.setTitle("Thông tin Huấn luyện viên");
	...
	public void showListHuanLuyenViens(List<HuanLuyenVien> list) {
		int size = list.size();
		Object[][] huanLuyenViens = new Object[size][5];
		for (int i = 0; i < size; i++) {
			huanLuyenViens[i][0] = list.get(i).getMaHuanLuyenVien();
			huanLuyenViens[i][1] = list.get(i).getHoTen();
			huanLuyenViens[i][2] = list.get(i).getTuoi();
			huanLuyenViens[i][3] = list.get(i).getDiaChi();
		}
		studentTable.setModel(new DefaultTableModel(huanLuyenViens, columnNames));
	}

	public void fillHuanLuyenVienFromSelectedRow() {
	...
	public void clearHuanLuyenVienInfo() {
		idField.setText("");
		nameField.setText("");
		ageField.setText("");
		addressTA.setText("");
//		gpaField.setText("");
	...
	public void showHuanLuyenVien(HuanLuyenVien huanLuyenVien) {
		idField.setText("" + huanLuyenVien.getMaHuanLuyenVien());
		nameField.setText(huanLuyenVien.getHoTen());
		ageField.setText("" + huanLuyenVien.getTuoi());
		addressTA.setText(huanLuyenVien.getDiaChi());
//		gpaField.setText("" + huanLuyenVien.getGpa());
	...
	public HuanLuyenVien getHuanLuyenVienInfo() {
		// validate student
...
//				|| !validateGPA()
		...
		try {
			...
			if (idField.getText() != null && !"".equals(idField.getText())) {
				huanLuyenVien.setMaHuanLuyenVien(Integer.parseInt(idField.getText()));
			}
			huanLuyenVien.setHoTen(nameField.getText().trim());
			huanLuyenVien.setTuoi(Byte.parseByte(ageField.getText().trim()));
			huanLuyenVien.setDiaChi(addressTA.getText().trim());
//			huanLuyenVien.setGpa(Float.parseFloat(gpaField.getText().trim()));
