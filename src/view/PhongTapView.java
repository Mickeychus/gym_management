...
public class PhongTapView extends JFrame implements ActionListener, ListSelectionListener {
...
//	private JTextField gpaField;
	...
	private String[] columnNames = new String[] { "Mã loại hình", "Tên loại hình", "Địa chỉ" };
...
//		layout.putConstraint(SpringLayout.WEST, sortStudentNameBtn, 115, SpringLayout.WEST, sortStudentGPABtn);
	...
	public void showListPhongTaps(List<PhongTap> list) {
		int size = list.size();
		Object[][] phongTaps = new Object[size][5];
		for (int i = 0; i < size; i++) {
			phongTaps[i][0] = list.get(i).getMaPhongTap();
			phongTaps[i][1] = list.get(i).getTenPhongTap();
			phongTaps[i][2] = list.get(i).getDiaChi();
		}
		studentTable.setModel(new DefaultTableModel(phongTaps, columnNames));
	}

	public void fillPhongTapFromSelectedRow() {
...
//			ageField.setText(studentTable.getModel().getValueAt(row, 2).toString());
			addressTA.setText(studentTable.getModel().getValueAt(row, 2).toString());
...
//		gpaField.setText("");
	...
	public void showPhongTap(PhongTap phongTap) {
		idField.setText("" + phongTap.getMaPhongTap());
		nameField.setText(phongTap.getTenPhongTap());
//		ageField.setText("" + phongTap.getTuoi());
		addressTA.setText(phongTap.getDiaChi());
//		gpaField.setText("" + phongTap.getGpa());
	...
	public PhongTap getPhongTapInfo() {
		// validate student
		if (!validateName()
//				|| !validateAge() 
				|| !validateAddress()
//				|| !validateGPA()
		...
		try {
			...
			if (idField.getText() != null && !"".equals(idField.getText())) {
				phongTap.setMaPhongTap(Integer.parseInt(idField.getText()));
			}
			phongTap.setTenPhongTap(nameField.getText().trim());
//			phongTap.setTuoi(Byte.parseByte(ageField.getText().trim()));
			phongTap.setDiaChi(addressTA.getText().trim());
//			phongTap.setGpa(Float.parseFloat(gpaField.getText().trim()));
			return phongTap;
		} catch (Exception e) {
